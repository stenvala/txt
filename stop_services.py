...
def get_bash_pids():
    lines = subprocess.run(["ps"], check=True, stdout=subprocess.PIPE, universal_newlines=True).stdout.split("\n")
    ttys = {}
    for i in lines:
        if not "ttys" in i:
            continue
...
for port in ports:
    try:
        data = subprocess.run(
            ["lsof", "-i", f"tcp:{port}"], check=True, stdout=subprocess.PIPE, universal_newlines=True
        )
    except:
